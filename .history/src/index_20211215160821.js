...
refreshButton.addEventListener('click', async (e) => {
  e.preventDefault();
  let list;
  getScores().then(async response => await response.result.map(el => list = `<li><span class="name">${el.user}</span> <span class="score">${el.score}</span> </li>`));
  console.log(list)
  scoreBoard.insertAdjacentHTML('beforeend', list);
});